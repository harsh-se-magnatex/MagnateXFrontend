...
/** Sync status fields written by the platform sync pipelines (see F4). */
export type SyncStatus = 'ok' | 'error';
export type SyncStatusFields = {
...
export type Post = {
  ...
  reactions: number;
  comments: number;
  commentsList?: { id?: string; message: string }[];
  shares: number;
...
export type InstagramPost = {
  ...
  likes: number;
  comments: number;
  commentsList?: { id?: string; text?: string; message?: string }[];
  saved: number;
  shares: number;
  engagementScore: number;
  engagementRate: number;
...
export type LinkedInPost = {
  ...
  commentsList?: {
    id?: string;
    /**
     * Full LinkedIn comment URN, e.g.
     * `urn:li:comment:(urn:li:activity:XXX,YYY)`. The activity portion
     * is *not* the same as the post's share URN, so we need to surface
     * the URN explicitly to use it as `parentComment` when replying.
     */
    commentUrn?: string;
    message: string;
  }[];
...
export type LiTrendKey = keyof Pick<
  LinkedInAnalytics,
  'followersTrend' | 'pageViewsTrend' | 'impressionsTrend' | 'engagementsTrend'
>;

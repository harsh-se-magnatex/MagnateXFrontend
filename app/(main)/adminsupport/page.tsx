...
function TicketCard({
  ...
  return (
    <div className="p-5 rounded-2xl bg-default border border-white/10 backdrop-blur-lg transition-expo">
      ...
      <p className="text-quaternary mb-4 leading-relaxed text-sm whitespace-pre-wrap">
        {msg.message}
      </p>

      <div className="flex flex-wrap items-center gap-3">
        <label className="font-medium text-tertiary text-sm">Status:</label>
        <select
          value={msg.status}
          onChange={(e) => onStatusChange(msg.userId!, msg.id, e.target.value)}
          disabled={updatingId === msg.id || !msg.userId}
          className="bg-default border border-white/20 text-white px-3 py-1.5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#00D1FF]/60 transition-expo"
        >
          <option value="open" className="text-default">
            Open
          </option>
          <option value="In-progress" className="text-default">
            In Progress
          </option>
          <option value="Resolved" className="text-default">
            Resolved
          </option>
        </select>

        {updatingId === msg.id && (
          <span className="text-xs text-[#00D1FF] animate-pulse">
            Updating...
          </span>
        )}
      </div>
  ...
  return (
    <section className="flex flex-col min-h-[320px]">
      <div className={`mb-4 pb-3 border-b border-white/10 ${accentClass}`}>
        <h2 className="text-section text-default">{title}</h2>
        <p className="text-sm text-tertiary mt-1">
          {tickets.length} request(s)
        </p>
      </div>
...
export default function AdminSupportViewer() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        ...
        showErrorToast(
          'Failed to get all users support messages. Please try again later.'
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      showErrorToast(
        'Failed to update support message status. Please try again later.'
      );
    } finally {
      setUpdatingId(null);
    }
  ...
  return (
    <div className="min-h-screen bg-[#0B1020] text-white px-4 sm:px-8 py-10">
      <h1 className="text-page-title text-default">Admin — Support Messages</h1>

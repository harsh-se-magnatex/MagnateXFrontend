...
function cellToneClass(kind: string): string {
  switch (kind) {
    ...
    case 'quick-create':
      return 'bg-sky-500/25 text-sky-400';
    case 'product-advert':
      return 'bg-fuchsia-500/25 text-fuchsia-300';
    case 'video-generation':
      return 'bg-cyan-500/25 text-cyan-400';
    case 'carousel':
      return 'bg-teal-500/25 text-teal-400';
    case 'bulk-create':
    case 'ai-engine':
      return 'bg-primary-purple/25 text-primary-purple';
    case 'empty':
                ...
                >
                  ...
                  >
                    <div className="flex flex-col gap-0.5">
                      ...
                      {festivalNames ? (
                        <span
                          className="mt-0.5 line-clamp-2 text-[10px] font-medium text-amber-300"
                          title={festivalNames}
                        >
                          {festivalNames}
                        </span>
                    ...
                    >
                      ...
                      >
                        ...
                        {forceRunEnabled && day.date >= todayIso ? (
                          <div
                            className="border-t border-cyan-500/30 bg-cyan-500/5 p-1.5"
                          ...
                          >
                            <button
                              ...
                              className="inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-cyan-500/40 bg-cyan-500/15 px-2 py-1.5 text-[11px] font-bold text-cyan-300 transition hover:bg-cyan-500/25 disabled:cursor-wait disabled:opacity-70"
                            >
                              {sharedVideoRunning ? (
                                <Loader2
...
export default function AIPlanPage() {
  ...
  const handleSavePlatforms = useCallback(async () => {
    if (platformChoice.length !== platformLimit) {
      toast.error(`Select exactly ${platformLimit} platform${platformLimit === 1 ? '' : 's'}`);
      return;
    }
    ...
    try {
      ...
      const message = err && typeof err === 'object' && 'response' in err
        ? String((err as { response?: { data?: { message?: string } } }).response?.data?.message ?? 'Could not save AI Plan platforms')
        : err instanceof Error ? err.message : 'Could not save AI Plan platforms';
      toast.error(message);
    } finally {
    ...
    return (
      <div className="mx-auto flex max-w-lg flex-col items-center justify-center gap-4 px-6 py-24 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          AI Plan automation is available on Prime AI, Elite AI, and Legacy AI plans.
        </p>
        <Link
    ...
    <div className="mx-auto max-w-6xl space-y-5 px-4 py-8 sm:px-6">
      ...
      <header className="space-y-2">
        ...
        <p className="max-w-2xl text-sm text-muted-foreground">
          Your AI Plan calendar from start to end — rows are days, columns are the platforms locked for this billing cycle.
        </p>
        {range?.from && range?.to ? (
      ...
      {platforms.length === 0 && !loading ? (
        <div id="platform-selection" className="border border-border bg-card px-5 py-8 text-center">
          <Share2 className="mx-auto mb-3 h-8 w-8 text-muted-foreground" />
          <p className="text-sm font-medium text-foreground">
            Select platforms for this AI Plan cycle
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            Select exactly the number included with your tier. Accounts can be connected later; the selection locks until renewal.
          </p>
          <div className="mx-auto mt-5 grid max-w-xl gap-3 sm:grid-cols-3">
            {(Object.keys(PLATFORM_LABEL) as AIPlanPlatform[]).map((platform) => {
              const checked = platformChoice.includes(platform);
              return (
                <label key={platform} className={cn('cursor-pointer rounded-xl border border-border p-4 text-left', checked && 'border-primary bg-primary/10')}>
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={checked}
                    disabled={!checked && platformChoice.length >= platformLimit}
                    onChange={() => setPlatformChoice((current) => checked ? current.filter((item) => item !== platform) : [...current, platform])}
                  />
          ...
          <p className="mt-4 text-xs text-muted-foreground">Your tier requires exactly {platformLimit}. This choice cannot change during the billing cycle.</p>
          <button
            type="button"
            onClick={() => void handleSavePlatforms()}
            disabled={savingPlatforms}
            className="mt-4 inline-flex rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {savingPlatforms ? 'Creating AI Plan…' : 'Lock selection and create AI Plan'}
          </button>
        </div>
      ) : null}

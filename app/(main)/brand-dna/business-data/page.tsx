...
const previousPathname = useRef(pathname);
    ...
    try {
      for (const item of snapshot) {
        ...
        try {
          ...
          if (humanFailure) {
            humanRejectedCount++;
            // Rejected files must not remain in the local uploaded-memory
            // collection, so they cannot be retried or persisted later.
  ...
  const [removingPhotoPath, setRemovingPhotoPath] = useState<string | null>(
    null
  );

  const handleRemovePhoto = async (path: string) => {
    if (removingPhotoPath) return;

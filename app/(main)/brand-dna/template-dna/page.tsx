...
export default function TemplateDnaPage() {
  ...
  useEffect(() => { void getTemplateDna().then(data => setProfiles(Object.fromEntries((data as TemplateDnaProfile[]).map(p => [p.platform, p])))).catch(() => showErrorToast('Could not load Template DNA.')); }, []);
  ...
  return <div className="mx-auto max-w-6xl pb-20"><Link href="/brand-dna" className="mb-6 inline-flex items-center gap-2 text-sm text-secondary transition-colors hover:text-default"><ArrowLeft className="size-4"/>Brand DNA</Link>
    <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full bg-primary-purple/10 px-3 py-1 text-xs font-semibold text-primary-purple"><Sparkles className="size-3.5"/>Brand consistency</div><h1 className="text-3xl font-bold tracking-tight text-default sm:text-4xl">Template DNA</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-secondary">Upload two or more examples per platform, extract the recurring visual rules, and keep control of every platform’s layout.</p></div><Link href="/brand-dna" className="text-sm font-semibold text-primary-purple hover:underline">Back to Brand DNA</Link></div>
    <p role="status" className="mt-3 text-sm text-secondary">{message}</p>
    <div className="mt-8 space-y-6">{platforms.map(platform => { const profile = profiles[platform]; const count = profile?.referenceAssets.length ?? 0; const locked = !!busy || !!profile?.extractionId; const statusLabel = profile?.status ? profile.status.replaceAll('_', ' ') : 'Not configured'; const ready = profile?.status === 'ready'; return <section key={platform} className="overflow-hidden rounded-3xl border border-default bg-card shadow-sm">
      ...
      </div><aside className="h-fit rounded-2xl border border-default bg-element p-4"><div className="mb-3 flex items-center gap-2 text-sm font-semibold text-default"><WandSparkles className="size-4 text-primary-purple"/>How it works</div><ol className="space-y-3 text-xs leading-5 text-secondary"><li><span className="mr-2 font-bold text-primary-purple">1</span>Upload 2–8 recognizable examples.</li><li><span className="mr-2 font-bold text-primary-purple">2</span>Extract the recurring visual rules.</li><li><span className="mr-2 font-bold text-primary-purple">3</span>Review the preview and adjust measurements.</li></ol>{count < 2 && <p className="mt-4 rounded-xl bg-amber-500/10 p-3 text-xs text-amber-800">Add {2 - count} more {2 - count === 1 ? 'image' : 'images'} to unlock extraction.</p>}</aside></div>
    </section>; })}</div>

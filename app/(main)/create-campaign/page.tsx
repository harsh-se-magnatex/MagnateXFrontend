  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Forward the auto-mode picker fields so a user whose set was
        // seeded by the auto-mode cron sees the "AI picked this for you"
        // badge + reason on the chosen card.
        loadSuggestionSet(set.suggestions, set.maxDays, {
          autoSeeded: set.autoSeeded,
          pickedSuggestionId: set.pickedSuggestionId,
          pickedReason: set.pickedReason,
        });
      } catch (err) {
  ...
  const handleGenerateSet = useCallback(
    async (size?: number) => {
      ...
      try {
        ...
        // User clicked "Generate ideas" \u2014 wipe any prior auto-seed badge
        // by passing no picker meta. The badge only re-appears if the
        // server returns a set with `autoSeeded: true`.
        loadSuggestionSet(set.suggestions, set.maxDays);
        toast.success(
...
type SuggestionGalleryProps = {
  ...
  /** True when the loaded set was generated by the auto-mode seed and
   *  the picker chose one of the cards. Drives the per-card badge. */
  autoSeeded: boolean;
  /** Id of the auto-picked card; nothing renders the badge when this is
   *  null or no card matches. */
  pickedSuggestionId: string | null;
  /** Justification the picker returned for choosing the card above. */
      ...
      {suggestions.length > 0 && (
        <>
          {autoSeeded && pickedSuggestionId && (
            <section className="glass-card rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4 flex items-start gap-3">
              ...
              <div className="text-xs text-emerald-900">
                <p className="font-bold">Auto-mode picked an idea for you</p>
                <p className="mt-1 text-emerald-800">
                  These {suggestions.length} ideas were generated by your
                  AI plan. The highlighted card below is the one auto-mode
                  scheduled this week
                  {pickedReason ? (
                    <>
                      {' '}
                      \u2014 <span className="italic">&ldquo;{pickedReason}&rdquo;</span>
                    </>
                  ) : (
                    '.'
                  )}
...
type SuggestionCardProps = {
  ...
  onRegenerate: () => void;
  onSelect: () => void;
  /** True only on the ONE card auto-mode picked. Adds an emerald border,
   *  a "AI picked this" pill, and (when supplied) the picker's reason
   *  underneath the description. */
  isAutoPicked?: boolean;
  /** Picker justification \u2014 only rendered when `isAutoPicked` is true. */
  pickedReason?: string | null;
};
...
function SuggestionCard(props: SuggestionCardProps) {
  ...
  return (
    <div
      className={cn(
        'group relative flex h-full flex-col rounded-3xl border bg-white p-5 shadow-sm transition hover:shadow-md',
        isAutoPicked
          ? 'border-emerald-300 ring-2 ring-emerald-200 hover:border-emerald-400'
          : 'border-slate-200 hover:border-indigo-200',
        isRegenerating && 'opacity-70'
      )}
    >
      {isAutoPicked && (
        <span className="absolute -top-2.5 left-4 inline-flex items-center gap-1 rounded-full bg-emerald-600 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white shadow-sm">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          {isAutoPicked && pickedReason && (
            <p className="mt-2 rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[11px] italic text-emerald-900">
              Why: {pickedReason}
            </p>
          )}
        </div>

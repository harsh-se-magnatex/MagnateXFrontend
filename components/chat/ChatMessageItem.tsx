...
export function ChatMessageItem({ message }: ChatMessageItemProps) {
  const isDismissed = useChatStore(
    (s) => Boolean(message.id && s.dismissedToolResults[message.id])
  );
  ...
  if (message.role === 'user') {
    return (
      <div className="flex justify-end px-4">
        <div className="max-w-[85%] rounded-2xl rounded-br-md bg-primary px-3 py-2 text-sm text-primary-foreground whitespace-pre-wrap">
          {message.content}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex justify-start px-4">
      <div
        className={cn(
          'max-w-[90%] rounded-2xl rounded-bl-md bg-muted px-3 py-2 text-sm text-foreground',
          refused && 'border border-dashed border-muted-foreground/40 bg-transparent italic text-muted-foreground'
        )}
      >
